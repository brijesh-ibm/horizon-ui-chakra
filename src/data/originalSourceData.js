...
export const getReviewsCount = (productName) => {
  return (
    OriginalSourceData.data.filter((review) => review.product === productName)
      .length * 100
  );
};
...
export const getReviewsCountGroupedBySourceAndSentiment = (productName) => {
  return OriginalSourceData.data
    ...
    .reduce((group, review) => {
      ...
      sentiments === "positive"
        ? (group[data_source].positive += 100)
        : (group[data_source].negative += 100);
      return group;
    }, {});
};
...
export const getReviewsCountGroupedByEntityAndSentiment = (productName) => {
  return OriginalSourceData.data
    ...
    .reduce((group, review) => {
      ...
      sentiments === "positive"
        ? (group[Entity].positive += 100)
        : (group[Entity].negative += 100);
      return group;
    }, {});
};
